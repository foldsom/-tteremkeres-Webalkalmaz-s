...
public static class DbSeeder
{
    ...
    {
        const string seedRelativePath = "database/restaurants_seed.json";

        var currentDirectory = new DirectoryInfo(Directory.GetCurrentDirectory());
        var baseDirectory = new DirectoryInfo(AppContext.BaseDirectory);

        return TryFindInParents(currentDirectory, seedRelativePath)
            ?? TryFindInParents(baseDirectory, seedRelativePath);
    }

    private static string? TryFindInParents(DirectoryInfo? startDirectory, string relativePath)
    {
